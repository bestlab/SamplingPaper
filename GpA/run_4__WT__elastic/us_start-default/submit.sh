...
if [ -f state0.cpt ]; then
    ...
    options="-multi $ng -maxh 24 -plumed -replex 1000 -s tpr/topol.tpr -x traj.xtc -pin on -rdd 1.7 -cpi state.cpt"
else
    options="-multi $ng -maxh 24 -plumed -replex 1000 -s tpr/topol.tpr -x traj.xtc -pin on -rdd 1.7"
fi
