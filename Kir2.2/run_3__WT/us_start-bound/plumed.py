...
h = u.select_atoms("resname PIP2 and name RP*")
...
#restraint: RESTRAINT ARG=dA,dB,dC,dD AT={},{},{},{} KAPPA=5e2,5e2,5e2,5e2
restraint: RESTRAINT ARG=dA AT={} KAPPA=5e2
PRINT STRIDE=1000 ARG=* FILE=COLVAR
"""

windows = np.linspace(2.6,4.5,16)
for i, e in enumerate(windows):
	open("plumed.{}.dat".format(i), "w").write(header+atoms+footer.format(windows[i], windows[i], windows[i], windows[i], windows[i]))
